				...
				<div class="content_inner">
					<div class="tit_area">
						...
						<span class="tit_btn_area">
							<router-link class="btn_mid_type01" to="/channel/insert">채널 등록하기</router-link>
						...
						<li class="channel-item" :data-code="channel.code" v-for="channel in channels">
							<div class="list link " @click="toggleChannel">
								...
								<div class="btn_edit_area">
									<button type="button"  class="btn_small01 write btn-channel-modi" @click="goChannelEdit($event, channel.code)" >수정</button>
...
export default {
	...
	methods: {
		...
		goChannelEdit: function (e, id) {
			console.log(id, 'channel edit id')
            this.$router.push({name: 'ChannelUpdate', params: {id: id}});
			// var form = $('<form>', {
			// 	method: 'post'
		...
		delChannel: function (id) {
			...
			if (confirm("채널을 삭제 하시겠습니까?")) {
				axios.post(API_URL+"/channel/delete.dat", data)
				.then((response) => {
					if (response.data > 0) {
						this.read__channels();
					}
