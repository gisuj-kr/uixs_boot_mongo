<template>
	...
	<div v-if="processingProgress > 0 && processingProgress < 100" class="progress-container">
		<div class="progress-text">파일 정보 수집 중... {{ processingProgress }}%</div>
							...
							<li style="margin-top: 10px;">
                                <dl class="horizontal-dl">
                                    ...
                                    <dd class="button-group">
										<!-- 주 단위 필터링 버튼들 -->
										<button type="button" class="round-button" @click="filterByLastWeek(1)">1주일</button>
										<button type="button" class="round-button" @click="filterByLastWeek(2)">2주일</button>
										<button type="button" class="round-button" @click="filterByLastWeek(3)">3주일</button>

                                        <button type="button" class="round-button" @click="filterByDate(1)">1개월</button>
                                        <button type="button" class="round-button" @click="filterByDate(2)">2개월</button>
                                        <button type="button" class="round-button" @click="filterByDate(3)">3개월</button>
                                        <button type="button" class="round-button" @click="sortByDate">최신순</button>
                                        <button type="button" class="round-button" @click="resetData">초기화</button>
                                    </dd>
                                </dl>
                            </li>
						</ul>
								...
								>
									<div class="menu_tit on">
										<div class="row-color-info" v-if="listVersion == 'tobe'">
											<div><span class="row-color-info__renewal-1st"></span>1차리뉴얼</div>
											<div><span class="row-color-info__renewal-2st"></span>2차리뉴얼</div>
											<div><span class="row-color-info__renewal-1st--modify"></span>1차개선</div>
											<div><span class="row-color-info__self--modify"></span>개선작업</div>
											<div><span class="row-color-info__as-is--modify"></span>as-is_개선</div>
										</div>
										<button 
											type="button"
											...
											@click="accordion($event, index)">{{EXCEL_JSON_HEADER[index]}}<span class="group-item-count">({{data.length}}개)</span></button>
									</div>
									...
									<!-- 아직 로딩되지 않은 그룹을 위한 플레이스홀더 -->
									<div v-else class="lazy-placeholder">
										<div class="placeholder-content">
											<div class="placeholder-text">
												스크롤하면 {{data.length}}개 항목이 로드됩니다...
											</div>
...
export default {
	...
	data: {
		...
		worker: null, // WebWorker 인스턴스
		processingProgress: 0, // 진행률
		storageChannel: localStorage.getItem('channel'),
		listVersion: 'tobe',
		refsArray: [],
		compoVisible: false,  // data-box 가시성 상태
      	observer: null,
		// 지연 로딩 관련 속성
		lazyLoading: {
			visibleGroups: new Set(),        // 현재 보이는 그룹들의 인덱스
			observer: null,                  // Intersection Observer 인스턴스
			rootMargin: '200px',             // 미리 로딩할 여백 (200px 미리 준비)
	...
	mounted: async function () {
		...
		$('.mobile_view').resizable({
			...
			start: (event, ui) => {
				...
				this.resizing = true; // html 미리보기 팝업창 리사이징 여부
			},
			stop: function (event, ui) {
				...
				setTimeout(() => {
					this.resizing = false; // html 미리보기 팝업창 리사이징 여부
				})
			}
		});
	...
	methods: {
		...
		// 검색 결과 하이라이트와 함께 해당 그룹으로 이동
		highlightSearchResultAndScroll() {
			if (this.searchText.trim() === '') return;
			
			// 검색 결과가 있는 첫 번째 그룹 찾기
		...
		// 메모리 사용량 최적화 - 보이지 않는 그룹 언로드
		optimizeMemoryUsage() {
			const viewportHeight = window.innerHeight;
			const scrollY = window.scrollY;
			
			// 현재 화면에서 너무 멀리 떨어진 그룹들 언로드
		...
		// 개발자 도구용 - 모든 통계 출력
		// console 창에 $vm0.debugLazyLoading() 실행
		debugLazyLoading() {
			const perfStats = this.getPerformanceStats();
			console.group('🚀 지연 로딩 상태');
			console.log('📊 그룹 통계:', perfStats.groups);
			console.log('📋 행 통계:', perfStats.rows);
			console.log('🎯 보이는 그룹:', Array.from(this.lazyLoading.visibleGroups).sort());
			console.log('💾 메모리 사용량:', `${perfStats.rows.percentage}% 로드됨`);
			console.groupEnd();
			...
			const callback = (entries) => {
				entries.forEach(entry => {
					...
					if (entry.isIntersecting) {
						// 그룹이 보이게 되면 visibleGroups에 추가
						this.lazyLoading.visibleGroups.add(groupIndex);
						console.log(`그룹 ${groupIndex} (${this.EXCEL_JSON_HEADER[groupIndex]}) 로드됨`);
					} else {
						// 그룹이 화면에서 멀어지면 제거 (메모리 절약)
						// 단, 너무 자주 제거하면 깜빡임이 있을 수 있으므로 선택적으로 사용
						// this.lazyLoading.visibleGroups.delete(groupIndex);
					}
				});
			};
		...
		observeGroups() {
			this.$nextTick(() => {
				...
				console.log(`${elements.length}개 그룹 관찰 시작`);
			});
		},
		...
		// 고동시성 최적화된 Vue3 Options API 코드 (WebWorker 제거, 날짜 정보 보장)
		// 1) 고동시성 처리 함수
		...
		async fetchLastModifiedOptimized(url) {
			...
			try {
				...
				setTimeout(() => controller.abort(), 1000); // 1초 타임아웃
		...
		async loadExcelWithLastModified() {
			...
			try {
				...
				console.log(`고동시성으로 ${uniqueUrls.length}개 URL 처리 시작...`);
				...
				console.log(`HTTP 요청 완료: ${endTime - startTime}ms`);
				...
				for (const { sheetName, rows } of allSheetData) {
					...
					this.EXCEL_JSON_DATA.push(rows);
				}
				...
				// 지연 로딩 초기화
				this.$nextTick(() => {
					this.preloadInitialGroups(3); // 첫 3개 그룹 미리 로드
					this.initLazyLoading();
				});
				
			} catch (e) {
				console.error('엑셀 처리 중 오류:', e);
			} finally {
				// setTimeout( () => {
					this.loading(false);
				// }, 4000);
		...
		filterByDate(months) {
			...
			console.log(`${months}개월 이내 수정된 파일: ${this.getFilteredCount()}개`);

			// 필터링 후 지연 로딩 재설정
			this.resetLazyLoading();
		},

		// 7-1) 최근 N주일 내 수정된 파일 조회
		filterByLastWeek(week = 1) {
			...
			weekAgo.setDate(weekAgo.getDate() - weekNo); // N주일 전
			...
			console.log(`최근 ${week}주일 내 수정된 파일: ${this.getFilteredCount()}개`);
		...
		resetData() {
			this.resetToOriginal();
			console.log('데이터 및 카테고리 순서 초기화 완료');
		...
		searchHandler: function () {
			...
			// 검색 결과가 있는 그룹들을 즉시 로드 (검색 결과를 바로 보여주기 위해)
		...
		// html-list-componet 의 ref 저장 메서드
		setRefs(el, index) {
			if (el) {
				this.refsArray[index] = el;
			}
		},
		// Intersection Observer 초기화 및 .data-box 관찰 시작
		initDataBoxObserver() {
			// Observer 옵션 설정
			const options = {
				root: null,           // 뷰포트를 기준으로
			...
			const callback = (entries) => {
				...
				entries.forEach(entry => {
					if (entry.target.classList.contains('menu_wrap')) {
						// data-box가 보이면 isVisible = true, 안보이면 false
						setTimeout(() => {
		...
		// .data-box 요소들을 찾아서 Observer에 등록
		observeDataBoxes() {
		...
		loadAllForBrowserSearch() {
			console.log('브라우저 검색을 위해 모든 그룹 로드 시작...');
			...
			this.$nextTick(() => {
				setTimeout(() => {
					const stats = this.getLazyLoadingStats();
					console.log(`모든 그룹 로드 완료: ${stats.loaded}/${stats.total}`);
					
					// 사용자에게 안내
					alert('모든 그룹이 로드되었습니다. 이제 Ctrl+F로 전체 검색이 가능합니다.');
				}, 1000);
			});
		},
		...
		handleKeyboardSearch(event) {
			if ((event.ctrlKey || event.metaKey) && event.key === 'f') {
				if (!this.isAllGroupsLoaded) {
					console.log('Ctrl+F 감지 - 자동으로 모든 그룹 로드');
					...
					this.$nextTick(() => {
						setTimeout(() => {
							...
							console.log('로딩 완료 - Ctrl+F 검색 가능');
						}, 800);
					});
				}
			}
		...
		showSearchLoadingNotification() {
			...
			notification.innerHTML = `
				<div class="notification-content">
					...
					<span>전체 검색을 위해 데이터를 로드하는 중...</span>
				</div>
			`;
