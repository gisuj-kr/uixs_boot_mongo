<template>
<div class="modal_layer_pop work_type" id="vue-popup" @click="closeWithDim" v-if="active" role="dialog" aria-modal="true">
    <div class="modal_layer_inner" style="background:#fff">
        <div class="pop_tit add-btn-edit">
            <button class="btn_edit" title="수정" @click="goModifyRequest">수정&#9998;</button>
            <a href="#none" class="btn_pop_close" title="레이어팝업 닫기" @click="popupClose"></a>
        </div>
                ...
                <li class="cont">
                    <div class="work_factor">
                        <dl>
                            ...
                            <dd>
								<div class="work-ia-path" v-if="workRequestData.path != '' && workRequestData.path"><span class="tit-for-path">경로</span> : {{ workRequestData.path }}</div>
...
export default {
    ...
    methods: {
        ...
        editDate: function () {
            ...
            axios.post(`${API_URL}/work/request_change_state.dat`, param)
            .catch(error => {
                console.log(error);
            });
...
//				}
            ...
            axios.post(`${API_URL}/work/request_change_state.dat`, sendData)
            .then(response => {
                
                eventBus.event.loadList(['request', 'process']);
