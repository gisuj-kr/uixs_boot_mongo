<template>
                    ...
                    <div class="input_section mt20">
                        <ul class="input_wrap" style="padding-top: 0;">
                            <li>
                                <div class="request-pg-checkbox-group">
                                    <div v-for="channel in channels" :key="channel.code">
                                        <input type="checkbox" name="channel" :id="channel.code" :value="channel.code"
                                            v-model="checkedChannelCodes" readonly> <label
                                            :for="channel.code">{{ channel.name }}</label>
                                    </div>
                                </div>
                                <div v-if="Object.keys(checkedChannel).length > 0 && checkedChannelCodes.includes('SH_PA')"
                                    class="chennel-sub-radio">
                                    <template v-for="(tab, tabIndex) in checkedChannel.ia_tabs" :key="tabIndex">
                                        <div v-if="tab.ia_file.trim() != ''">
                                            <input type="radio" name="channel-sub" :value="tabIndex" :id="tab.tab_name"
                                                v-model="checkedChannelSubIdx"> <label :for="tab.tab_name">파트너 {{
                                                tab.tab_name }}</label>
                                        </div>
                                    </template>
                                </div>
                            ...
                            <li class="input_area" style="margin-top: 20px"
                                v-if="checkedChannels.length === 1 && checkedChannelCodes.includes('SH_PA')">
                                <div class="input_area-label-group">
                                    <label for="request_title">경로</label> <button type="button" class="btn-open-ia"
                                        @click="openTreeModal">IA
                                        열기</button>
                                </div>
                                <input type="text" id="path" name="path" v-model="formData.path"
                            ...
                            <li class="input_area">
                                ...
                                <textarea id="request_content" name="request_content" placeholder="내용을 입력해주세요."
                            ...
                            <li class="input_area half">
                                <div class="input-inner half">
                                    ...
                                    <input type="text" id="request_date" name="request_date" placeholder="날짜를 선택해주세요"
                                        class="datepick" data-type="date" autocomplete="off"
                                        :value="formData.request_date">
                                </div>
                                <div class="input-inner half">
                                    <label for="request_complete_date">완료 요청일</label>
                                    <input type="text" id="request_complete_date" name="request_complete_date"
                                        placeholder="날짜를 선택해주세요" class="datepick" data-type="date" autocomplete="off"
                            ...
                            <li class="input_area">
                                ...
                                <ul class="box_type_check">
                                    <li>
                                        <input type="checkbox" v-model="need_workers" name="need_workers" id="team01"
                                            value="plan">
                                        <label for="team01">기획</label>
                                    </li>
                                    <li>
                                        <input type="checkbox" v-model="need_workers" name="need_workers" id="team02"
                                            value="design">
                                        <label for="team02">디자인</label>
                                    </li>
                                    <li>
                                        <input type="checkbox" v-model="need_workers" name="need_workers" id="team03"
                                            value="publish">
                                        <label for="team03">퍼블</label>
                                    </li>
                                </ul>
                            </li>
                            <li class="input_area edit_pop"
                                style="width: 100%; box-shadow: none; padding: 0 0; top: 0; display: none;">
                                <label for="">첨부파일
                                    <span>
    ...
    <tree-component v-if="showTreeModal" @close="showTreeModal = false" :checked-channel="checkedChannel"
        :checked-channel-sub-idx="checkedChannelSubIdx"></tree-component>
        ...
        modify__request: async function (e) {
            ...
            try {
                ...
                const axiosPromises = channelArr.map((code) => {
                    ...
                    return axios.post(API_URL + '/work/modify_request.dat', sendData);
                });
