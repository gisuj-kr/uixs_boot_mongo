...
export default {
	...
	mounted: function () {
		...
		(async () => {
			...
			if (this.vmode == 'modify') {
                ...
                this.checkPermission(this.userid)
				.then(hasPermission => {
					if (hasPermission) {
	...
	methods: {
        ...
        async checkPermission(id) {
            try {
				// withCredentials: true - 배포시에는 제거
				// axios.defaults.withCredentials = true; // 전역 설정 (필요시 사용)
                const response = await axios.get(`${API_URL}/member/checkPermission?id=${id}`, { withCredentials: true });
            ...
            .then(hasPermission => {
                if (hasPermission) {
                    ...
                    if (confirm('사용자를 삭제 하시겠습니까?')) {
						axios.post(API_URL+'/member/member_delete.dat', null, {params: {id: this.member.id}})
						.then(response=>{
							if(response.data) {
								alert('삭제가 완료되었습니다.');
								this.$router.push(`/member/${this.page}`);
							}
							else {
		...
		modifyMemberData() {
			...
			axios.post(API_URL+'/member/member_update.dat', null, {params: paramData})
			.then(response => {
				if (response.data) {
					alert('사용자 수정이 완료 되었습니다.');
					this.$router.push(`/member/${this.page}`);
					// this.$router.push({name: 'MemberList', params: {page: this.page}});
